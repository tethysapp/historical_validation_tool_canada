...
function init_map() {
	...
	map = new ol.Map({
		...
		view: new ol.View({
			center: ol.proj.fromLonLat([-104.895627,57.479509,3]),
			zoom: 5
		})
	});
...
//                $('#obsdates').removeClass('hidden');
				...
				get_time_series(watershed, subbasin, streamcomid, stationcode, stationname, startdate, province);
...
function get_time_series(watershed, subbasin, streamcomid, stationcode, stationname, startdate, province) {
    $('#forecast-loading').removeClass('hidden');
    $('#forecast-chart').addClass('hidden');
    $('#dates').addClass('hidden');
    $.ajax({
